...
def convolutional_layer(data, kernel_size, bias_size, pooling_size):
    ...
    pooling = max_pool_2x2(linear_output,pooling_size)
    return pooling


# 构建池化层
def max_pool_2x2(linear_output, pooling_size):
    return tf.nn.max_pool(linear_output, ksize=pooling_size, strides=pooling_size, padding="SAME")
...
def convolutional_neural_network(data):
    ...
    full_conn_w_shape = [15 * 12 * 64, 2000]
    out_w_shape = [2000, n_ouput_layer]

    bias_shape1=[32]
    bias_shape2=[64]
    full_conn_b_shape = [2000]
    ...
    with tf.variable_scope("cnn_layer1") as layer1:
    ...
    with tf.variable_scope("cnn_layer2") as layer2:
